...
class SQLService {
  ...
  Future saveRecord(ShopItemModel data) async {
    await this.db?.transaction((txn) async {
      var qry =
          'INSERT INTO shopping(name, price, image,category,description,rating,fav) VALUES("${data.name}",${data.price}, "${data.image}", "${data.category}", "${data.description}",${data.rating},${data.fav ? 1 : 0})';
  ...
  Future addToCart(ShopItemModel data) async {
    await this.db?.transaction((txn) async {
      var qry =
          'INSERT INTO cart_list(shop_id, name, price, image,category,description,rating,fav) VALUES(${data.id}, "${data.name}",${data.price}, "${data.image}", "${data.category}", "${data.description}",${data.rating},${data.fav ? 1 : 0})';
      int id1 = await txn.rawInsert(qry);
      return id1;
    });
  }
